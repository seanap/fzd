#!/usr/bin/env bash
# fzd.sh — ls loop + preview + Enter opens files + Ctrl+F global search
# Keys:
#  - Left  : up in parent dir (preselect child)
#  - Right : open into child dir
#  - Enter : dir -> exit & cd; file -> open in editor then resume
#  - Esc   : exit
#  - Ctrl+F: open global search overlay (rooted at / by default)
...
# ----------------- CONFIG DEFAULTS -----------------
# ---- load user config, then fall back to defaults (safe with set -u) ----
FZD_CONF_FILE="${FZD_CONF_FILE:-$HOME/.fzd/fzd.conf}"
[[ -f "$FZD_CONF_FILE" ]] && source "$FZD_CONF_FILE"

: "${FZD_GLOBAL_BACKEND:=cache}"           # auto|locate|cache|live
: "${FZD_GLOBAL_MINLEN:=2}"
...
FZD_DEBUG="${FZD_DEBUG:-0}"
FZD_GLOBAL_ROOT="${FZD_GLOBAL_ROOT:-/}"          # filesystem root for Ctrl+F overlay
FZD_EXCLUDES="${FZD_EXCLUDES:-.git,node_modules,.cache,.venv,__pycache__}"
FZD_PREVIEW_DEPTH="${FZD_PREVIEW_DEPTH:-2}"
FZD_PREVIEW_MAX_LINES="${FZD_PREVIEW_MAX_LINES:-200}"
FZD_PREVIEW_TIMEOUT="${FZD_PREVIEW_TIMEOUT:-2}"

# ------- theme (Catppuccin-ish; only for left list labels) -------
c_reset=$'\033[0m'
c_dir=$'\033[38;2;249;226;175m'   # yellow
c_file=$'\033[38;2;205;214;244m'  # white-ish

# ----------------- FZF hardening -----------------
FZF_BIN="$(type -P fzf 2>/dev/null || command -v fzf || true)"
[[ -x "$FZF_BIN" ]] || { echo "fzd: ERROR: fzf not found" >&2; exit 1; }

# Avoid env defaults overriding our binds
FZF_DEFAULT_OPTS=
...
list_entries() {
  ...
  local path base
  # dirs: include dot dirs explicitly (not using dotglob)
...
if [[ "${1:-}" == "--_global_list" ]]; then
  ...
  : "${FZD_GLOBAL_MINLEN:=3}"              # start searching at N chars
  : "${FZD_GLOBAL_MAXRESULTS:=3000}"       # limit inside locate
  : "${FZD_GLOBAL_PATHS:=/etc /opt /srv /mnt /home/$USER}"  # whitelist for UI
  : "${FZD_GLOBAL_XEXCLUDES:=proc,sys,dev,run,proc/*,sys/*,dev/*,run/*,mnt/c,mnt/d,snap,lost+found,var/lib/docker}"
  : "${FZD_GLOBAL_FULLPATH:=0}"
  : "${FZD_LOCATE_DBS:=}"

  # don’t spawn anything for very short queries
  ...
  # whitelist regex: ^(/etc|/opt|/srv|/mnt|/home/ubu)(/|$)
  roots_re=""
  for r in "${roots[@]}"; do
    r="${r%/}"; [[ -z "$r" ]] && r="/"
...
_open_in_editor() {
  local file="$1"
  [[ -f "$file" ]] || return 0
  # attach editor to the tty; block until it exits
...
_run_global_overlay() {
  ...
  if [[ "$backend" == "locate" ]]; then
    ...
    # very old fallback; nothing to do
...
main() {
  ...
  local HIST_FILE="${XDG_STATE_HOME:-$HOME/.local/state}/fzd/query-history"
  ...
  while :; do
    ...
    # Defensive: fzf may print [query, key, ...] with --print-query + --expect
    ...
    # Wait briefly for the temp-file write (in case fs/flush is slow)
    for _i in {1..20}; do
      [[ -s "$CTRL" ]] && break
      sleep 0.01
    done
    ...
    # If nothing was written and no expected key, treat as Esc
